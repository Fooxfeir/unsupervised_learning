...
def estimate_latent_scale(vae_encoder, dataloader, device):
    vae_encoder.eval()
    mus = []
    with torch.no_grad():
        for images in dataloader:
            images = images.to(device)
            mu, _ = vae_encoder(images)
            mus.append(mu)
    mus = torch.cat(mus, dim=0)
    # desvio-padrão médio das dimensões
    std_emp = mus.std(dim=0).mean()
    return 1.0 / std_emp
...
def train_latent_diffusion(
    ...
    latent_scale = estimate_latent_scale(vae_encoder, dataloader, device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, images in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                if deterministic_encoding:
                    # Deterministic encoding: use only the mean
                    latents = mu * latent_scale
                else:
                    # Stochastic encoding: reparameterization trick
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='Use deterministic VAE encoding (mu only) instead of sampling from the posterior'
    )
